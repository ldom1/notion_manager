...
# Read data
logger.info("Reading data loaded from timesheets_summary.xlsx...")
...
my_project_df_gr = my_project_df.groupby(
    ["client", "project", "task", "username", "week"], as_index=False
)["volume"].sum()
...
NotionTempsProjectDatabase().delete_items_in_database()
...
for _, row in tqdm(my_project_df_gr.iterrows()):
    response = NotionTempsProjectDatabase().add_one_timeslot(row)
